...
// connect to database
$db = mysqli_connect('localhost', 'root', '', 'bluespec6');
	...
	if (count($errors) == 0) {
		...
		if (mysqli_num_rows($results) == 1) { // user found
			...
			if ($logged_in_user['user_type'] == 'admin') {
				...
				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				$_SESSION['id']  = "You are now logged in";
...
if (isset($_POST['read_btn'])) {
	markRead($_POST['messageID']);
}

function markRead($mID)
{
	global $db;
	$query = "UPDATE contacts SET markRead= 1 WHERE id='".$mID."'";
	if ($db->query($query) === TRUE) {
...
{
	global $db;
	$query = "DELETE FROM users WHERE id='".$mID."'";
	if ($db->query($query) === TRUE) {
		echo "New record created successfully";
	  } else {
...
$imagepdf = file_put_contents("random text", $file); 
